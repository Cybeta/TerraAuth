...
- ✅ 持久化落盘：玩家存档 / 审计 / 封禁写入 SQLite，重启后仍在（封禁不再重启即失效）；**世界改动（挖 / 放 / 液体 / 电线 / 执行器）增量落盘并在启动时回放，玩家建筑重启不丢**
- ✅ 世界文件（`.wld`）：`WorldPath` 指定即可**用真实世界开服**；`WorldExportPath` 启用后停机 + 空服导出整份 `.wld`（写出器带写后读回校验 + 原子替换 + `.bak` 滚动）

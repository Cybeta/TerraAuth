...
- ✅ 战斗 / 生存权威：受伤（117）非负校验 + 服务端生命结算 → 死亡广播（118）；**敌怪 / Boss 接触伤害由服务端判定**（32px + 60tick 免伤帧，下发 117 表现 + 16 权威血量）；复活（12）由服务端划定复活点；弹幕命中判定与掉落物拾取（22）均在服务端结算

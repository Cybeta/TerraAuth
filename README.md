...
| Phase 2 | 物品 & 战斗权威 | ✅ 物品 ID / 堆叠校验 + SSC 背包对账 + **箱子内容服务端持有**（开箱下发权威内容、包 32 校验落盘）；单次伤害上限 + DPS 窗口；受伤→死亡→复活（`DamagePlayerCommand` / `KillPlayerCommand` / `RespawnCommand`）+ 弹幕命中判定 + 掉落物拾取（包 151 拾取通知 + 包 22 归属同步）均服务端结算 |

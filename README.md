...
- ✅ 世界文件（`.wld`）：`WorldPath` 指定即可**用真实世界开服**；未指定时按 `WorldSize` **程序化生成三档尺寸**（`Small` 4200×1200 / `Medium` 6400×1800 / `Large` 8400×2400，与原版三档一致）；`WorldExportPath` 启用后停机 + 空服导出整份 `.wld`（写出器带写后读回校验 + 原子替换 + `.bak` 滚动）

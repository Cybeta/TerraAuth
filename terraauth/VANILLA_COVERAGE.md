...
| 功能 | 现状 | 备注 |
|---|---|---|
| 敌怪生成 / AI | 已**简化**实现：每 60 tick 在随机玩家附近刷史莱姆（上限 8），朝最近玩家水平移动 + 重力 | 无原版刷怪规则（生物群系 / 昼夜 / 事件）、无 NPC 专属 AI 与接触伤害 |
| 弹幕 / 掉落物服务端模拟 | 已**跟踪**：服务端持有实体与生命周期（掉落物重力落地、弹幕生存期到期），到期弹幕由服务端补发包 29 | 客户端视觉仍靠 21 / 27 实时中继；掉落物拾取（包 22）与弹幕命中判定未做 |
| 箱子内容管理 | 包 32 可中继，但服务端不维护箱子内容 | 打开箱子看到的仍是客户端本地数据 |
| 电路 / 液体 | 未实现 | 原版分别约数千行（连线图 + 信号传播 / 逐格流动与压力） |
| 受伤 / 死亡 / 复活流程 | 117 / 118 仅中继，无服务端结算与复活 | 需 `DamagePlayerCommand` / `RespawnCommand` 与包 12 复活分支 |
| 世界进度 / Boss 事件 | 包 7 仅在进服时下发一次 | |

> 结论：服务端已达「可进服 + 地形可见 + 彼此可见（含移动 / 受伤 / 增益 / 弹幕 / 掉落物）+ 挖放砖权威 + 聊天 + 时间与 NPC 同步 + 简化刷怪」，
> 但**仍不是完整可玩的原版服务器**（缺拾取 / 命中判定、箱子内容、电路液体、死亡复活）。

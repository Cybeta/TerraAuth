...
3. **NPC 同步省略 ai（与发送 ai=0 等价）**：客户端在 ai 位未置位时会把该 ai 显式置 0，
   故「省略」与「发送 0」等价；当前服务端没有真实 NPC ai，NPC 动作由**客户端自身 AI** 驱动（原版同样如此）。
   已补齐**同步锚点**（史莱姆王：位置 + 体型×锚点）并把目标字段改为 **255（显式无目标）**。
   遗留：NPC 的**增益**（buff）未同步。

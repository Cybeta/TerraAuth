...
| 背包同步 | 5 | 槽位 / 堆叠 / 物品校验；**SSC 下服务端持有唯一真相，并以「守恒事务」接受客户端整理结果**：包 5 与服务端权威值不一致时不逐包回正，而是暂存进 15 tick（≈250ms）窗口，窗口到期按 **(物品, 前缀) 总堆叠数守恒**整体提交或回滚 —— 拖拽 / 整理 / 拆分 / 合并 / 交换守恒即提交（否则玩家无法整理背包），凭空造物或销毁必然破坏总量而被回滚并回写权威值；该分流按客户端行为噪声处理，**不计违规**（`Vanilla_ClientIsNotKicked_After_NormalInventoryOps`）。开箱期间的包 5 归入箱子事务（见「箱子内容」行）。**合成 / 纯消耗已纳入判据**：产物净增量由原版配方表 `RecipeTable`（3301 条 / 34 组，含原版 `CreateReverseWallRecipes` / `CreateReversePlatformRecipes` 派生的「墙 → 块 / 平台 → 材料」）解释，并按 `requiredTile` + **表内全部环境标志**（`needWater` / `needHoney` / `needLava` / `needSnowBiome` / `needGraveyardBiome` / `needMechdusa` / `needTorchGodsFavor`）**校验合成站与环境**（`CraftingEnvironmentSampler` 镜像原版 `AdjTiles`：可达区域 X ±5 / Y ±3 图格、`Recipe.TileCountsAs` 等价图格、`liquid > 200`；并镜像原版 `SceneMetrics.ScanTiles` 的**固定 169×124 图格**场景扫描区，得雪原图格 ≥ 1500 → `ZoneSnow`、墓碑数 − 向日葵数/2 ≥ 28 → `ZoneGraveyard`，与客户端分辨率无关）；机械三王取世界特性（`WorldState.MechdusaFeature`，程序化世界恒 false）、火把神恩取玩家解锁状态（包 4 `TorchFlags` bit2 → `PlayerRuntime.UnlockedBiomeTorches`）。「只减不增」的消耗（药水 / 投掷物 / 丢弃）在背包侧放行；带非 0 前缀的造物、材料不足、**不在合成站旁**（含超出可达区域）、**不在雪原 / 墓地**、世界无该特性或玩家未解锁、超出窗口基准（外部 `give` / 拾取 / 开袋）的消耗均回滚。已知边界：同窗口多级合成与 13 条静态不可解的 Lesion 家具配方回滚。SscEnabled=false 时服务端不再持有背包权威，包 5 直接写入。**断线时按玩家名把背包 / 生命 / 法力落盘，新会话进服时回读**（`PlayerProfileCodec`，否则重进即清空） | `Vanilla_InventorySlot_InvalidSlot_IsRejected` / `Vanilla_InventoryDrag_Is_Accepted_And_Conserved` / `Vanilla_InventoryCraft_Is_Accepted_And_Conserved` / `Vanilla_InventoryCraft_Without_Station_Rolls_Back` / `Vanilla_InventoryConsume_Is_Accepted` / `Vanilla_InventoryCraft_Short_Of_Materials_Rolls_Back` / `CraftingEnvironmentSampler_Tracks_Station_And_Liquids` / `CraftingEnvironmentSampler_Tracks_Snow_And_Graveyard_Biomes` / `CraftingEnvironmentSampler_Tracks_Mechdusa_And_TorchGodsFavor` / `InventoryTransaction_Graveyard_Recipe_Follows_Biome` / `InventoryTransaction_Snow_Biome_Recipe_Follows_SceneMetrics` / `InventoryTransaction_TorchGodsFavor_Recipe_Follows_Player_Unlock` / `InventoryTransaction_Mechdusa_Recipe_Follows_World_Feature` / `InventoryTransaction_Commits_Derived_Reverse_Wall_Recipe` / `RecipeTable_Extracts_Key_Vanilla_Recipes` / `AntiCheat_Ssc_Forged_InventorySnapshot_Is_Rolled_Back` / `AntiCheat_Ssc_Forged_ArmorSnapshot_Is_Rolled_Back` / `Vanilla_TreasureBag_DoesNotSilenceOtherSlots` / `Vanilla_ClientIsNotKicked_After_NormalInventoryOps` / `Vanilla_SscInventory_Survives_Reconnect` / `PlayerProfileCodec_RoundTrips_Inventory_And_Vitals` |
...
| **NPC 生成 / 同步** | 23 (SyncNPC) | 60Hz 循环内**逐 NPC 分档**下发：Boss / 近身（3 格内）NPC 逐 tick，其余 20Hz（原版令牌桶为普通 ≈2 包/秒、Boss ≈12Hz）；包内含索引 / netID / 位置 / 速度 / 朝向 / **原版 `ai[0..3]`** / **难度覆盖段**（bitsB.bit2 + float，专家 2 / 大师 3）/ **玩家数段**（bitsB.bit0 + byte，专家以上按人数缩放 Boss 生命时的「按几人缩放」）；**状态变化（含 ai / 朝向）才发**，未变化按 1s 心跳补发（保证新入服玩家能看到静止 NPC）；已满血形态省略生命段。**NPC 生命 / 伤害按原版 `NPC.ScaleStats` 全链缩放**：专家+困难模式弱怪补强（`NpcHardmodeScaling`：强度 < 80 / 击败世纪之花后 100，整数除法口径；Boss / `lifeMax ≥ 1000` / `DontDoHardmodeScaling` 豁免、投射物型只放大伤害）→ 难度曲线（专家 ×2 / 大师 ×3）→ 在线人数（`NpcPlayerCountScaling`：balance 1 / 1.35 / 1.9167…，只放大生命上限，含撒旦军队 0.857 与火星暴乱 / 旧日军团 `1+(n−1)×0.2`）。客户端按包内难度段 / 玩家数段 + 包 7 的困难模式位重算同一上限，血条才与服务端一致 | `Vanilla_Npc_Is_Synced_To_Client` / `Vanilla_NpcSync_SendsOnChange_AndSkipsUnchanged` / `Vanilla_NpcSync_Carries_Vanilla_Ai_State` / `Vanilla_Npc_Sync_Carries_Difficulty_Override` / `NpcUpdate_Difficulty_Override_RoundTrips_In_Codec` / `SpawnedNpc_Scales_Life_And_Damage_By_Difficulty` / `ExpertMode_Contact_Damage_Is_Scaled_Once` / `NpcHardmodeScaling_Tops_Up_Weak_Mobs` / `NpcHardmodeScaling_Skips_Bosses_Excluded_Types_And_Projectile_Npcs` / `NpcPlayerCountScaling_Follows_Vanilla_Curve` / `SpawnedBoss_Scales_By_Difficulty_And_PlayerCount` |
...
| **对话 NPC（服务端持有 + 中继）** | 40 | `Byte 玩家 + Int16 talkNPC`（-1 = 未对话）；仅做边界校验（越界按 `sync` 类审计、**不计违规**）→ `SetTalkNpcCommand`：目标不存在 / 非活跃 / 非城镇 NPC 一律回落 -1，**状态真正变化才**标记 → 向其他玩家广播包 40（原版不向他人转发自己的对话目标，此处用于他人可见的对话指向） | `SetTalkNpc_Tracks_State_And_Marks_Only_On_Change` / `SyncTalkNpc_RoundTrips_In_Codec` / `Vanilla_TalkNpc_Is_Tracked_Server_Side_And_Relayed_To_Others` / `Vanilla_Invalid_TalkNpc_Is_Rejected_Without_Violation` |
| **NPC 增益（服务端持有 + 全量下发）** | 53 / 54 | 客户端包 53（`Int16 npcId + UInt16 type + Int16 time`）并入该 NPC 增益列表（`Dictionary<int,int>`，上限 5 槽，满则懒替换首个；`time <= 0` 表示移除）→ 包 54 向全体玩家**全量**下发该 NPC 增益列表。遗留：增益的**效果**（减速 / 持续伤害等）与计时递减尚未模拟，仅维护列表与同步 | `ApplyNpcBuff_StoresBuff_And_MarksBuffsChanged` / `ApplyNpcBuff_Rejects_InvalidTargets` / `NpcBuffSync_RoundTrips_In_Codec` |
...
| **箱子内容（服务端持有 + 持久化）** | 31 / 32 / 34 / 85 | 开箱校验（存在 / 距离）→ 服务端逐槽下发权威内容（包 34 + 包 32×N）；**包 32 走「箱子守恒事务」**：暂存进 15 tick 窗口，到期按「**玩家背包 ∪ 该箱子**」的 (物品, 前缀) 总堆叠守恒整体提交或回滚（箱内整理 / 交换守恒即提交，凭空造物即回滚）。**背包 ↔ 箱子的单槽转移**（客户端同时发包 5 清空背包侧 + 包 32 填入箱子侧）**两侧归入同一事务**结算 —— 若背包侧单独走背包事务，会因背包总量减少被判不守恒而回滚，存入箱子永远不生效。**取用箱内材料合成**同理被配方解释（产物落背包），但箱子侧**不放行纯消耗**（防客户端用包 32 静默清空箱内物品）。**包 85 快速堆叠**：客户端上报来源槽位 + smartStack，服务端按当前打开的箱子**自行规划**装箱（不接受客户端最终快照）；包 85/32 均校验槽位 / 堆叠 / 物品 / 距离。改动**登记增量落盘**（重启后回放，按索引 + 坐标校验） | `Vanilla_ChestOpen_Sends_Authoritative_Contents` / `Vanilla_ChestItem_Is_Applied_Authoritatively` / `Vanilla_ChestItem_Rearrange_Is_Conserved_And_Committed` / `Vanilla_ChestDeposit_FromInventory_Is_Accepted_And_Conserved` / `Vanilla_ChestQuickStack_IsAccepted` / `AntiCheat_Ssc_Forged_ChestSnapshot_Is_Rolled_Back` / `Vanilla_ChestItem_Invalid_Slot_Is_Rejected` / `Vanilla_ChestItem_OutOfReach_Is_Rejected` / `Vanilla_ChestContent_Survives_ServerRestart` |
...
| **背包守恒事务（SSC）** | 5 / 32 | 背包槽位写入**不逐包回正**（拖拽 / 整理是多包窗口），而是进 15 tick 窗口按 (物品, 前缀) 总堆叠守恒**整体提交或回滚**：合成 / 消耗 / 反向配方由配方表解释（含合成站与环境），**纯消耗（药水 / 投掷物 / 丢弃）放行、凭空造物回滚**。**开袋单独结算**：袋槽 + 客户端掷骰的战利品按战利品池 + 单次上限校验（净增**跨全部前缀**求和，带词缀装备照常到手）后与其余槽位解耦提交；附带的**金币**按 `基础值 × [0.8, 5]` 设上界；**未建模的袋失败关闭**（净减少解释不了就回滚保袋）。**放置被拒**时登记退回凭据，窗口结算把这次无产出凭据的减少退回（物品不被吞） | `TreasureBag_ClientRoll_IsCommitted_AndLootKept` / `TreasureBag_OpenCommits_EvenWithUnrelatedForgeryInWindow` / `TreasureBag_CoinReward_IsCommitted_WhenWithinRange` / `TreasureBag_CoinReward_AboveRange_OnlyCoinsRolledBack` / `TreasureBag_CoinReward_RealMachineRoll_IsAccepted` / `TreasureBag_LootWithPrefix_IsKept` / `TreasureBag_LootWithPrefix_StillRespectsStackLimit` / `TreasureBag_CursorPickup_WithoutLoot_RollsBack_AndKeepsBag` / `InventoryTransaction_Rescues_RejectedPlacement_Removal` / `InventoryTransaction_Keeps_Removal_When_Item_Was_Dropped` |
| **放置（方块 / 墙 / 家具）** | 17 (action 1/3) / 79 | 放置包携带的是**图格 / 墙 ID**，而背包里是**物品 ID**（工作台：物品 36 ↔ 图格 18；泥土 2 / 0；木头 9 / 30）——服务端用 `TileToItemTable`（图格 524 项 / 墙 203 项，**值为物品 ID 集合**）反查回**放置物品**再校验背包：计入本窗口未结算的暂存值、**优先手持槽**，命中任意一个候选即可（同一图格可由数十个物品放置，如工作台图格 18 有 65 个候选）；**校验命中哪件、扣减就扣哪件**，且扣减与图格写入在**同一提交单元**（区块写锁内，锁序 SectionLocks → PlayersLock）。反查不到（该图格 / 墙不由任何物品放置）→ `tile_item_unknown` 拒绝；背包没有 → `item_not_in_inventory` 拒绝（失败方向安全）。覆盖包 79（`TileObject.CustomPlace`）与包 17 action 1（普通方块）/ action 3（墙）两条链路。**家具类图格按「图格物件」整体处理**（`TileObjectTable`，389 项）：锚点 = 点击格 − `Origin`，按 `Width × Height` 写满 footprint 并写入与原版 `TileObject.Place` 同口径的帧（只写点击单格、不写帧会让客户端已摆好的整件家具与服务端分叉 —— 真机表现为「放下的工作台看起来碎了」并在相邻格被 `tile_already_exists` 误拒）；占用检查同样按整个 footprint；破坏时整件移除并只掉 1 份。**放置被拒**时不但拒绝，还登记「本窗口退回凭据」：窗口结算时把这次无产出凭据的物品减少退回（权威值原样保留并回写该槽，客户端拿回物品），同时**补发该格权威单格**（包 20）消除客户端「幽灵方块」—— 否则会出现「方块放下去马上被销毁」（物品没了、地图上也没有） | `Place_WorkBench_Accepts_Any_Candidate_Item_Variant` / `Place_WorkBench_Command_Consumes_NonMinimal_Variant` / `Place_WorkBench_Without_InventoryItem_Is_Rejected` / `Place_Dirt_Tile_Maps_To_DirtBlock_Item` / `Place_Wood_Tile_Maps_To_Wood_Item` / `Place_Unmapped_Tile_Is_Rejected` / `Packet17_Action1_PlaceTile_Uses_Same_Mapping_And_Consumes_Item` / `Packet17_Action3_PlaceWall_Uses_Wall_Mapping_And_Consumes_Item` / `Place_Consumes_From_SelectedSlot_First` / `TileToItems_Maps_Known_Tiles` / `WallToItems_Maps_Known_Walls` / `TileToItemTable_Header_Counts_Match_Dictionaries` / `InventoryTransaction_Rescues_RejectedPlacement_Removal` / `InventoryTransaction_Keeps_Removal_When_Item_Was_Dropped` / `InventoryTransaction_Rescue_Does_Not_Touch_Other_Items` / `InventoryTransaction_Ignores_Stale_Rejected_Placement` / `Vanilla_TilePlace_On_Occupied_Tile_Resends_Authoritative_Tile` / `TileObjectTable_WorkBench_Entry_Matches_Vanilla_Geometry` / `TryGetFrame_WorkBench_Matches_Vanilla_Place_Formula` / `Place_WorkBench_Writes_Whole_Footprint_With_Frames` / `Place_WorkBench_Rejects_When_Footprint_Cell_Occupied` / `Place_Normal_Tile_Still_Writes_Single_Cell_Without_Frames` / `Break_WorkBench_Removes_Whole_Footprint_And_Drops_Once` / `Break_Legacy_WorkBench_Without_Frames_Falls_Back_To_Connected_Footprint` / `Break_Normal_Tile_Still_Removes_Single_Cell` / `TileObjectTable_Header_Counts_Match_Dictionary` |
...
| 箱子内容管理 | 已**服务端持有并持久化**：开箱（包 31 → `OpenChestCommand`，在仿真提交阶段建立**绑定连接会话**的打开会话）下发权威内容；包 32 / 包 85 经校验后进入**箱子守恒事务**（15 tick 窗口，按「玩家背包 ∪ 该箱子」总量守恒提交 / 回滚），改动增量落盘（重启回放）。**背包 ↔ 箱子的转移**在同一事务内结算，快速堆叠由服务端规划 | 未实现包 33（完整箱子同步）/ 箱子命名 / 上锁 / 放置新箱子（包 34 语义为 SyncPlayerChestIndex，非放置）；客户端 UI 依赖服务端逐槽包 32。打开会话按 `(PlayerId, SessionId)` 绑定，并在**包 31 负坐标（关箱请求）**、**离开交互距离（每 tick 距离复核）**、断线三处关闭：旧连接（槽位被复用）的延迟箱子命令与断线清理不再影响新连接 |
...
| 世界改动持久化 | 已**实现增量落盘 + 启动回放**：图格改动（含方块 / 墙 / 液体 / 电线 / 执行器）与**箱子内容**（包 32）按 1Hz 落盘（单批 8192，集合溢出降级为全图扫描），重启后叠加回基准世界；**世界进度**（Boss 击杀 / 困难模式 / 事件与种子开关，82 个位）单独一行落库并启动回放（`WorldProgressCodec` 版本化位图，**不复用包 7 的字节** —— 那里混有 `SscEnabled` / `CloudBgActive` 两个非进度字段），换种子时随世界改动一起清 | 箱子内容回放按「索引 + 坐标」双校验，基准世界被替换时坐标不符则跳过；崩溃最多丢 1 秒改动（进度同为 1Hz，判据是「与上次成功落盘的字节比对」而非会被包 7 广播消费掉的进度脏位）。**SSC 玩家档案**另有「**500ms 比对、变更才写**」（挂机玩家 0 次写库，崩溃最多丢 ~0.5 秒）+ 停机补一次，断线仍走即时落盘；SQLite 连接 `synchronous=FULL`（**提交即 fsync**，把「崩服少丢存档」落到磁盘层面） |
...
3. **NPC 同步已携带原版 `ai[0..3]`**：客户端在 ai 位未置位时会把该 ai 显式置 0，故「省略」与「发送 0」等价 ——
   ...
   遗留：未登记 aiStyle 的 NPC 动作仍由客户端自身 AI 兜底。
4. **NPC 增益已服务端持有并同步**：客户端包 53（AddNPCBuff）并入该 NPC 的增益列表（上限 5 槽，满则懒替换首个），
   由包 54（NpcBuffSync）向全体玩家全量下发 —— 见 §一「NPC 增益」行。
   遗留：增益的**效果**（减速 / 持续伤害等）与计时递减尚未在仿真中模拟，仅维护列表与同步。
5. **快照存储已是定长环形数组**（`SnapshotStore`，无元素搬移）—— 见 `Net/Snapshots/README.md`「快照存储线程安全」。
6. 原版客户端**不支持预测协议**，延迟只能靠快照频率缓解（不影响防作弊，见 `architecture.md` 约束）。
7. **玩家受伤：接触 / 下落伤害已改为服务端判定**（`SimulateCombat` + `ApplyPlayerDamage`，含原版免伤帧 —— 接触 / 弹幕 / 下落同档 40、伤害被压到 1 取 20，
...
8. **图格推送分两条路（已完成协议字段核对）**：**少量图格改动**（电路翻转执行器、液体混合等）走**包 20（TileSquare）**，
   按视口下发给附近玩家；**区块级地形**（离开出生点后的流送）走**包 10（TileSection）**。
   与原版一致：包 20 用于小矩形改动，包 10 用于区块下载。
9. **液体同步已按视口裁剪**，但仍为「每玩家全量过滤」（复杂度 O(玩家数 × 变更数)）；玩家数 / 变更数继续增大后需按区块分桶下发。
10. **Boss / 事件为简化模型**：眼魔（4）/ 魔焰眼（31）/ 蜂后（43）已按原版 `aiStyle` 移植运动与攻击节奏，
  其余 Boss 仍为直线追击、生命值为简化表；血月 / 日食为昼夜概率、入侵为配额刷怪，均非原版规则；
  掉落已收录 **19 个 Boss**（`Simulation/World/WorldState.cs` 的 `BossLoot`）：眼魔为**原版全表**（腐化 / 猩红两套经典掉落
  + 眼面具 1/7、望远镜 1/40 + 专家宝袋 3319 必掉），其余按 `Fixed` 简化规格（如世界吞噬者 → 恶魔矿 + 暗影鳞片、
  蜂后 → 蜂蜡 + 蜂枪、月亮领主 → 月矿 + 传送枪），未复刻原版掉落数据库的完整概率与条件。
11. **液体混合反应为简化模型**：已实现异种液体累计 ≥ 24 单位生成混合图格（水 + 岩浆 → 黑曜石 56、水 + 蜂蜜 → 蜂蜜块 229、
    岩浆 + 蜂蜜 → 松脆蜂蜜块 230、微光 + 任意 → 微光块 659，图格 ID 已完成协议行为验证）；遗留：仅在本格为空时生成
    （原版还允许覆盖可被黑曜石破坏的图格），且无液体压力模型。
12. **区块超帧上限已有兜底**：编码后超过 `UInt16`（65535）时按较长轴二分拆分再发（`NetworkHost.SendTileSectionAsync`），
    避免高熵区块直接抛异常中断登录；原版是同通道的降级压缩路径，此处用拆分替代。
13. **弹幕类型与伤害以服务端推导为主，但仍非完整原版模型**：包 27 的类型须落在当前权威武器 / 合法弹药允许集合内
    （`Simulation/Combat/WeaponProjectileTypeOf.cs`、`WeaponAmmoTypeOf.cs`、`AmmoTypeOf.cs`），远程须消耗背包中匹配
    `useAmmo` 的弹药；伤害按权威武器 / 弹药 / 前缀 / 增益与饰品推导（近战、魔法为精确推导，远程含合并弹药上界）。
    已收录远程及固定弹幕魔法的包 28 命中还必须关联同归属、存活且与目标 AABB 接触的服务端登记弹幕。
    遗留：多弹幕 / 随机弹 / 特殊弹药的完整规则、以及全部逐武器 `useTime` / `useAnimation` 数据表仍在补全；
    法力 / 增益已改为服务端跟踪与持有，但**玩家增益的效果**仍由客户端计算（服务端只维护列表），属简化模型。
14. **实体位置语义与物理常数已按原版对齐**：`position` = **碰撞盒左上角**，脚底 = `position.Y + height`；
...
- **已踩过的量纲坑**：`NpcStrike.Damage` 线上为 **Int16**（±32767），与默认上限 30000 几乎贴边（见 §四 第 1 条）。

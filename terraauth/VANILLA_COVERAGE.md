...
| 世界文件（`.wld`）加载 / 导出 | 已**实现**：`ServerConfig.WorldPath` 指定 `.wld` 即作为基准世界开服；`WorldExportPath` 非空则停机 + 空服周期**导出整份 `.wld`**（写出器：全部 11 段含 5..9 段的合法空编码，写后读回校验 + 原子替换 + `.bak` 滚动） | 已验证：**逐格 round-trip** + **按协议加载顺序的严格分段走查**（段起点递增、逐段位置断言、footer 用尽文件）。**客户端互操作性验证尚未完成**（本沙箱无法运行目标服务端，连其自建世界也崩溃） |

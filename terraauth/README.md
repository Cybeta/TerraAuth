...
| 模式 | `SscEnabled` | 行为 |
|---|---|---|
| 服务端持有（默认） | `true` | 服务端唯一真相：包 5 / 包 32 与服务端权威不一致时**不逐包回正**，而是暂存进 15 tick（≈250ms）窗口，到期按 **(物品, 前缀) 总堆叠数守恒**整体提交或回滚（背包按「背包自身」，箱子按「玩家背包 ∪ 该箱子」）。拖拽 / 整理 / 拆分 / 合并 / 交换守恒即提交；凭空造物或销毁必然破坏总量而被回滚并回写权威值。该分流按客户端行为噪声处理，**不计违规**。开箱期间包 5 归入箱子事务，使背包 ↔ 箱子转移原子结算 |
| 关闭 | `false` | 服务端不再持有背包权威：包 5 直接写入，`/give` 走「归属掉落物」路径，服务端权威特性（放置扣减、拾取入库、宝袋开袋）一并停用 |

> 已知边界（`SscEnabled=true`）：**合成与消耗物品会改变背包总量，当前仍会被判不守恒回滚** —— 原版配方表尚未建模，属后续 A4/P3 范围；对应地，`DestroySummonsOnWeaponRemoval` 等依赖服务端背包的联动也仅在 SSC 下生效。

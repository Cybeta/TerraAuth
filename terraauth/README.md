...
| `Simulation/World/` | 部分 | `Tile`/`TileMap`、`TileIdSets`（含 `tileFrameImportant`）、`WorldState`、`.wld` **解析器 + 写出器**（写出器与读取器布局严格对称，带写后读回校验 / 原子替换 / `.bak` 滚动；解析含最小合法世界 / 版本 / 魔数 / footer 测试）、包 10 `TileSection` 与包 20 `TileSquare` 编码均已实现；**程序化生成支持原版三档尺寸**（小 4200×1200 / 中 6400×1800 / 大 8400×2400，`ServerConfig.WorldSize`）并生成**分层地形**（噪声地表 / 洞穴 / 按深度分带矿脉 / 海滩与海水 / 地狱层 / 2×2 宝箱+战利品），层高与图格 ID 经协议行为验证；**地表树木（原版 `GrowTree` 树干部分）、地下生命水晶、生物群系（腐化 / 猩红 / 雪原 / 沙漠 / 丛林 / 发光蘑菇地 / 地牢）均已产出**；仍为简化的分层生成，**缺**神庙 / 蜂巢 / 大理石·花岗岩等结构体 |
...
| `Net/Transport` `PacketEncoder` | 部分 | 已实现 **43 类出站包**（握手链 2/3/4/7/8/9/10/12/49/129 + 权威与状态 5/13/14/16/17/18/20/21/22/23/27/28/29/31/32/34/35/36/40/42/50/54/65/73/79/85/117/118/151/162 + 包 82 的 NetText / NetLiquid 模块 + 包 15 `Snapshot`；按 case 统计）；包 10 `TileSection`（Deflate + 位标志 + RLE + 尾部列表）、**包 20 `TileSquare`（未压缩小矩形 + 逐格位标志/可选段）**、包 13 可选尾随段（挂载 / 回城 / 相机）已实现 |
| `Net/Transport` `PacketDecoder` | 部分 | 已解析 **40 个入站映射**（握手链 + 权威白名单 17 包 + 拾取 151·22 / 箱子 31·32·85 / 伤害 28·117·118 / 治疗 35 / 法力 42 / 增益 50·53 / 对话 NPC 40 / 传送 65·73 / 时间 18 / NPC 23 / 聊天与液体模块等）+ 包 15 `Snapshot`；其余统一 `UnknownPacket`，由 Vanilla-only Authority 默认拒绝 |
...
`ModCompat` 代码仅作为未来独立立项的兼容层保留，`TModLoaderCompat` 默认 `enabled: false`。当前生产为 **Vanilla-only**：仅原版客户端（协议 326）、不接受 TModLoader 握手、不注册 250-255、不转发自定义或未知包（未知包由权威层默认拒绝）。未来 MOD 工作必须单独完成协议、权限、限流、审计与测试设计，不能绕过当前权威提交与服务端最终状态同步链路。

> 完整范围与代码状态见 [`ModCompat/README.md`](ModCompat/README.md)。
...
| **P0** | ✅ 持久化异步化（`SqlitePersistence` 审计走有界 `Channel` + 批量落盘） | 中 | 低 |
...
| 模式 | `SscEnabled` | 行为 |
|---|---|---|
| 服务端持有（默认） | `true` | 服务端唯一真相：包 5 / 包 32 与服务端权威不一致时**不逐包回正**，而是暂存进 15 tick（≈250ms）窗口，到期按 **(物品, 前缀) 总堆叠数守恒**整体提交或回滚（背包按「背包自身」，箱子按「玩家背包 ∪ 该箱子」）。拖拽 / 整理 / 拆分 / 合并 / 交换守恒即提交；**合成**（材料减少 + 产物增加，由原版配方表 `RecipeTable` 解释，并按 `requiredTile` + 水 / 蜂蜜 / 岩浆**校验合成站与环境**）与**纯消耗**（只减不增，如药水 / 投掷物 / 丢弃）作为例外放行（箱子侧不放行纯消耗）；凭空造物、带前缀造物、材料不足或**不在合成站旁**的「合成」被回滚并回写权威值。该分流按客户端行为噪声处理，**不计违规**。开箱期间包 5 归入箱子事务，使背包 ↔ 箱子转移与「取用箱内材料合成」原子结算 |
| 关闭 | `false` | 服务端不再持有背包权威：包 5 直接写入，`/give` 走「归属掉落物」路径，服务端权威特性（放置扣减、拾取入库、宝袋开袋）一并停用 |

> 已知边界（`SscEnabled=true`）：合成校验**材料守恒 + 合成站（可达区域 X ±5 / Y ±3 图格，含等价图格）+ 水 / 蜂蜜 / 岩浆**，配方表 3301 条（含原版派生的「墙 → 块 / 平台 → 材料」）；但**不校验**依赖客户端分辨率场景度量或玩家解锁状态的标志（雪原 / 墓地 / 特殊种子 / 火把神恩）；同一窗口内的**多级合成**（先合成中间物再立刻用它合成成品，中间物净变化为 0）与 13 条静态不可解的 Lesion 家具配方会回滚（回滚方向安全）。对应地，`DestroySummonsOnWeaponRemoval` 等依赖服务端背包的联动也仅在 SSC 下生效。

...
> 已知边界（`SscEnabled=true`）：合成校验**材料守恒 + 合成站（可达区域 X ±5 / Y ±3 图格，含等价图格）+ 水 / 蜂蜜 / 岩浆**，但**不校验**依赖客户端分辨率场景度量或玩家解锁状态的标志（雪原 / 墓地 / 特殊种子 / 火把神恩）；同一窗口内的**多级合成**（先合成中间物再立刻用它合成成品，中间物净变化为 0）与未收录配方（反向墙 / 平台配方）会回滚（回滚方向安全）。对应地，`DestroySummonsOnWeaponRemoval` 等依赖服务端背包的联动也仅在 SSC 下生效。

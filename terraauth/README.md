...
| 模式 | `SscEnabled` | 行为 |
|---|---|---|
| 服务端持有（默认） | `true` | 服务端唯一真相：包 5 / 包 32 与服务端权威不一致时**不逐包回正**，而是暂存进 15 tick（≈250ms）窗口，到期按 **(物品, 前缀) 总堆叠数守恒**整体提交或回滚（背包按「背包自身」，箱子按「玩家背包 ∪ 该箱子」）。拖拽 / 整理 / 拆分 / 合并 / 交换守恒即提交；**合成**（材料减少 + 产物增加，由原版配方表 `RecipeTable` 解释）与**纯消耗**（只减不增，如药水 / 投掷物 / 丢弃）作为例外放行（箱子侧不放行纯消耗）；凭空造物、带前缀造物或材料不足的「合成」被回滚并回写权威值。该分流按客户端行为噪声处理，**不计违规**。开箱期间包 5 归入箱子事务，使背包 ↔ 箱子转移与「取用箱内材料合成」原子结算 |
| 关闭 | `false` | 服务端不再持有背包权威：包 5 直接写入，`/give` 走「归属掉落物」路径，服务端权威特性（放置扣减、拾取入库、宝袋开袋）一并停用 |

> 已知边界（`SscEnabled=true`）：合成只校验**「材料 → 产物」的守恒**，不校验合成站 / 环境条件（工作台、砧、水 / 岩浆等），故客户端仍可在任意位置合成；同一窗口内的**多级合成**（先合成中间物再立刻用它合成成品，中间物净变化为 0）与未收录配方（反向墙 / 平台配方）会回滚（回滚方向安全）。对应地，`DestroySummonsOnWeaponRemoval` 等依赖服务端背包的联动也仅在 SSC 下生效。

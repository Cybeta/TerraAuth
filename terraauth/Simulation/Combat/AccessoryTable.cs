// TerraAuth — 服务端权威伤害结算：饰品伤害修饰表（阶段 E-3，数据表补齐）
// 数据源：Terraria 1.4.5.8 原版 Player.ApplyEquipFunctional 的 currentItem.type 判定（ID 以 ItemID.cs 为准）。
...
// 注：日曜/月曜/天界石/天界壳共用原版 skyStoneEffects **标志**（同时佩戴多件也只 +10% 一次），
// 本表按件各自 +10 → 多件并戴时上界偏高（宽松方向，不误拒，仅减弱反作弊精度）。
...
/// <summary>饰品 ID → 伤害修饰（百分比 = 原版数值 × 100）。仅收「直接给伤害加成」的饰品；纯召唤伤害（徽章/卷轴类）不收。</summary>
public static class AccessoryTable
{
    ...
    {
        [489]  = new(0, 0, 0, 15),    // 巫师徽章 Sorcerer Emblem：魔法 +15%（Player.cs L16366）
        [490]  = new(0, 15, 0, 0),    // 战士徽章 Warrior Emblem：近战 +15%（L16370）
        [491]  = new(0, 0, 15, 0),    // 游侠徽章 Ranger Emblem：远程 +15%（L16374）
        [899]  = new(10, 0, 0, 0),    // 日曜石 Sun Stone：全伤害 +10%（白天，skyStoneEffects）
        [900]  = new(10, 0, 0, 0),    // 月曜石 Moon Stone：全伤害 +10%（夜晚/日食，skyStoneEffects）
        ...
        [1858] = new(0, 0, 10, 0),    // 狙击镜 Sniper Scope：远程 +10%

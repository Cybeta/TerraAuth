...
| `Tests/` | 部分 | 7 组验收测试（160 用例通过）；`VanillaFeatureTests` 以真实权威管线 + 真实 TCP 覆盖原版功能（矩阵见 [`VANILLA_COVERAGE.md`](VANILLA_COVERAGE.md)）；包 10 / 包 15 编解码回归、`WorldGenerator` 确定性测试、踢出与违规阈值触发、纠正包类型、插件 API 踢出与封禁、Hook 参数填充包数据、配置阈值启动映射与热重载、实体视图发布、区块分区锁并发安全（真实 TCP）、持久化往返（玩家 / 审计 / 封禁重启读回）已补，`.wld` 解析测试待补 |

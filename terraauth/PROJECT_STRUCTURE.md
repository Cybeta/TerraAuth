...
| `Compile` | `**\*.cs`（`Exclude="Tests\**;Examples\**;bin\**;obj\**"`） | 递归收拢所有源码，排除测试、示例与构建产物 |
...
**测试工程配置**：`Microsoft.NET.Test.Sdk 18.10.0` + `xunit 2.9.3` + `xunit.runner.visualstudio 3.1.5`，显式 `Compile` 列表（10 个测试文件），单一 `ProjectReference` → `..\TerraAuth.csproj`。
...
├── WorldManagement.cs            #   WorldManager：世界管理（列表 / 导入 / 创建 / 选择，namespace TerraAuth）
├── CoreAdapter.cs                # 核心类型 ↔ 插件/Mod 接口桥接（namespace TerraAuth.Plugins）
├── Diagnostics.cs                # 热路径诊断日志开关 DiagnosticLog（默认关闭，配 VerboseDiagnostics）
...
│   ├── Combat/                   #   服务端权威伤害结算 + 战斗数据表
│   │   ├── CombatResolver.cs     #     共享战斗判定 / 伤害公式
│   │   ├── NpcStats.cs           #     NPC 基础战斗属性（伤害 / 防御 / 生命上限）
│   │   ├── NpcVariantLifeTable.cs#     负 netID 变体生命上限表
│   │   ├── ItemDamageTable.cs    #     武器基础伤害表
│   │   ├── ItemDefenseTable.cs   #     装备防御表
│   │   ├── PrefixDamageTable.cs  #     武器前缀伤害倍率表
│   │   ├── BuffTable.cs          #     Buff / 药水战斗修饰表
│   │   ├── ArmorSetBonusTable.cs #     套装加成表
│   │   ├── ArmorPieceBonusTable.cs#    护甲单件伤害修饰表
│   │   ├── AccessoryTable.cs     #     饰品伤害修饰表
│   │   ├── AmmoDamageTable.cs    #     弹药攻击力表
│   │   ├── AmmoTypeOf.cs         #     弹药物品 → 弹药类型表
│   │   ├── WeaponAmmoTypeOf.cs   #     武器 → 弹药类型表
│   │   ├── WeaponProjectileTypeOf.cs#  武器 / 弹药 → 弹幕类型映射
│   │   ├── WeaponUseBehaviorTable.cs#  已登记武器的开火行为表
│   │   ├── ProjectileCapabilityTable.cs# 弹幕物理能力表（图格碰撞 / 额外更新）
│   │   └── SummonProjectileTable.cs#   召唤 / 哨兵弹幕类型表
│   ├── NpcAI/                    #   NPC AI（按原版 aiStyle 移植）
│   │   ├── WorldSimulator.NpcAi.cs#     NPC AI 主实现（partial）
│   │   └── NpcSizes.cs           #     实体碰撞盒尺寸表
│   ├── Projectiles/              #   弹幕行为（按原版字段 / aiStyle 移植）
│   │   └── WorldSimulator.Projectiles.cs# 弹幕行为主实现（partial）
│   ├── Spawn/                    #   刷怪（原版 NPC.Spawner 移植）
│   │   ├── SpawnRate.cs          #     刷怪频率（GetSpawnRate 移植）
│   │   ├── EnemySpawnPool.cs     #     敌怪刷怪池（SpawnAnNPC 移植）
│   │   ├── NpcCritterSet.cs      #     小动物集合与行为分派
│   │   ├── NpcNetIdMap.cs        #     负 netID → 基础 NPC 类型映射
│   │   └── BiomeMetrics.cs       #     场景度量（生物群系 / 深度带判定）
│   └── World/                    #   世界数据模型
│       ├── Tile.cs               #     Tile / TileMap（含包 20 TileSquare 契约）
│       ├── TileIdSets.cs         #     瓦片 ID 集合（FrameImportant 等）
│       ├── TileDropTable.cs      #     图格 → 掉落物表
│       ├── TileEntity.cs         #     图格实体（WorldFile section 5）
│       ├── WorldState.cs         #     世界状态
...
│   └── WorldManagementTests.cs   #   世界管理（列表 / 导入 / 创建）
│
...
| 模块 | 状态 | 说明 / 待办 |
|------|------|-------------|
| `Protocol/` | 部分 | 已定义 47 个 `PacketId` 常量与对应包契约（握手链 + 权威白名单 + 快照包 15 等）；原版 200+ 包按需补充 |
...
| `Net/Transport` `PacketEncoder` | 部分 | 已实现 **43 类出站包**（握手链 2/3/4/7/8/9/10/12/49/129 + 权威与状态 5/13/14/16/17/18/20/21/22/23/27/28/29/31/32/34/35/36/40/42/50/54/65/73/79/85/117/118/151/162 + 包 82 的 NetText / NetLiquid 模块 + 包 15 `Snapshot`；按 case 统计）；包 10 `TileSection`、**包 20 `TileSquare`（未压缩小矩形）**、包 13 可选尾随段（挂载 / 回城 / 相机）已实现 |
| `Net/Transport` `PacketDecoder` | 部分 | 已解析 **40 个入站映射**（握手链 + 权威白名单 17 包 + 拾取 / 箱子 / 快速堆叠 / 伤害 / 死亡 / 治疗 / 法力 / 增益 / 对话 NPC / 传送 / 时间 / NPC / 聊天与液体模块等）+ 包 15 `Snapshot`；其余统一 `UnknownPacket`，由 Vanilla-only 权威层默认拒绝 |
...
| `Config/` | 已实现 | `ServerConfig`（含 `ModPolicy` 节 + `WorldPath` / `WorldSize`（小 / 中 / 大三档）/ `WorldExportPath` / `VerboseDiagnostics`）+ `FileSystemWatcher` 热重载；枚举以字符串读写 |
...
数字一律以 `dotnet test Tests/TerraAuth.Tests.csproj` **实跑结果**为准；最近一次基线记录：

| 项 | 值 |
|---|---|
| 用例总数 / 通过 / 失败 / 跳过 | **818 / 818 / 0 / 0** |
| 目标框架 / 配置 | `net10.0` / Debug |
| 执行日期 / 耗时 | 2026-09-19 / 1m56s |

> 口径权威来源是交接说明 `.trae/documents/TerraAuth 交接说明.md`（面向接手人的总览）。
> `README.md`、本文件、`VANILLA_COVERAGE.md` 等处的测试数字须与该处同步；**历史轮次记录**（如
> `OPTIMIZATION_BACKLOG.md` 各轮回溯、`RELEASE_NOTES.md` 已发布小节）保留当时的数字，不回改。

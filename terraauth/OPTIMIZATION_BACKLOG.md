...
> 最后更新：2026-09-19（W-2 第一步数据表收口；第二步 4 项全部落地；第三步 ServerAi：移动表 + **本体位置权威与命中几何覆盖 62 / 62**（剩余族全部纳入）；835/835 通过）
...
2. **`ServerAi`**：本体跟随 / 驻守 / 索敌 / 节奏 / 暴击在服务端，包 27 只用于创建登记；
   ...
   **第三步第三批已落地（2026-09-19）：`PositionBound` 族锚点约束** —— `SummonMovementTable` 新增
   `PositionBoundAnchorOf` / `PositionBoundTolerance(200)`；包 27 更新路径对 `Mode == PositionBound`
   的本体（831 / 970 的家点、626-628 的父节）做**锚点约束**：坐标超出 200px 容忍圈 → 吸附回锚点并清零速度，
   圈内照常接受。测试：`ServerAi_PositionBound_Body_Snaps_Back_To_Anchor`（含 831 吸附 / 圈内合法偏移不吸附 /
   970 同族 / 626 锚到 625 头节 / 无头节时失败放行 / ClientDriven 对照）。
   **为什么只能做到「锚点约束」而不是「精确复刻」**：这些本体的原版位置依赖**客户端视觉状态** ——
   831/970 的家点含公转相位 `miscCounterNormalized`、`bodyFrame` 头饰偏移与 `gfxOffY`；626-628 依赖**客户端驱动**的父节。
   服务端不建模这些量，逐帧复刻不可能，故退化为「必须在锚点容忍圈内」——容忍圈 200px 远大于合法偏移
   （831/970 公转半径 ≤ 8+12×排号；626-628 距头节 ≤ 3×16×1.5≈72），**不会误拒合法位置**。
   **第三步第四批已落地（2026-09-19）：AI_062 族位置权威（混合模型）+ 命中几何收紧** ——
   经确认采用**混合模型**：`ProjectileEntity.ServerPosition` / `ServerVelocity` 是服务端**自算的判定用**位置，
   表现用的 `Position` 仍是客户端包 27 上报值并原样广播 → **主人视角无抖动**、客户端无法影响判定。
   · `WorldSimulator.SimulateSummonMovement`（新阶段 5.55，仅 `>= ServerAi` 生效）：对 **373 / 375 / 407 / 423 / 613 / 963**
     复刻「待命点（主人中心上方 60，407/375/963 各有偏移）+ 惯性 `v=(v×20+Δ)/21` + 距离分档速度上限 + 召回提到 15 +
     超远传送（>2000 → 主人中心）」，**不复刻**索敌 / 冲刺 / 绕飞（服务端无目标选择）。
   · 命中几何：`SummonMovementTable.HitGeometryTolerance = 1200`，包 28 的召唤通道对**有 ServerPosition 的本体**
     要求目标 NPC 落在该位置的**可达圈**内，否则 `ProjectileNotColliding` ——
     **堵住「把本体挪到全图任意 NPC 旁再报命中」**（此前任意本体可命中全图任意 NPC）。
     取 1200 是因为服务端只复刻跟随、不动战斗机动，必须留足余量避免误拒；远小于世界尺度（小图宽 67200px），
     故仍能拦住跨图作弊。想收紧需服务端能复刻目标选择（后续 / `ServerShots`）。
   · **未自算位置的族 `ServerPosition` 为 null → 不判、失败放行**，逐族接管。
   测试：`ServerAi_Ai062_Body_Server_Position_Follows_Owner`（ClientDriven 不碰位置 / 停在待命点 /
   朝主人靠拢且**表现坐标不被改写** / 召回提速 / 超远传送）与
   `ServerAi_Ai062_Hit_Requires_Npc_Within_Server_Reach`（5000px 外拒绝、600px 内结算、387 不受影响）。
   **第三步第五批已落地（2026-09-19）：AI_026 与 AI_067 两族纳入** —— 服务端自算位置从 6 个本体扩到 **28 个**：
   · **AI_026**（191-194 / 266 / 390-392 / 1094 / 1113）与 **AI_067**（393-395 / 758 / 833-835 / 951 / 1022 /
     1093 / 1112 / 1118）都是**贴地族**：按 `StanceBase + StanceStep × 同类序号` 在主人**面朝方向**横向站位
     （逐类型基础距离与步长已入表：AI_026 = 40/40；AI_067 逐类型 25/20、45/40、25/40、55/30、40/20、34/20…），
     竖直取「主人脚下同一水平线」。
   · `SummonMovementInfo` 新增 `StanceBase` / `StanceStep` 两个字段（把原先只写在注释里的「待命点公式」正式入表）。
   · 召回触发器补上贴地族的第二条：**`|Δy| > 300` 也召回**（原版 L67174）。
   · **两处已知近似**（写判定前必读）：①贴地族**不做重力与图块碰撞** —— 真正的贴地要读图格（`SectionLocks`），
     而本阶段持有 `ProjectilesLock`，反序取区块锁会死锁，故竖直用「主人脚下」近似（丘陵地形误差 = 坡度高度，
     远小于 1200px 容忍圈）；②横向排队用「同类存活序号」近似原版 `minionPos`。
   · 测试：`ServerAi_Ground_Families_Follow_Owner_Stance`（站位 370 / 按步长排到 410 / Flinx 385 / 表现坐标不被改写 /
     主人掉头后翻到另一侧 / 几何收紧对该族生效）。
   **第三步第六批已落地（2026-09-19）：剩余 Body 族全部纳入 —— 覆盖 62 / 62** ——
   · 新增 `SummonMovementInfo.AnchorOffsetX` / `AnchorOffsetY`（待命点相对主人中心的偏移，X 按朝向取正负），
     把原先散在代码里的逐类型偏移（AI_062 的 375/407/963、317、387/388、533、623、755/946、759、864、
     831/970、1119）**正式入表**；`SimulateSummonMovement` 因此收敛成「贴地族走 StanceBase、
     飞行族走 AnchorOffset、哨兵冻结、龙节段跟随头节」四条统一规则。
   · **哨兵**：冻结在生成点（原版只有 ≤ 约 30px 的垂直落体/贴顶微调，服务端不读图格故忽略）。
   · **星尘龙节段 626-628**：第二遍跟随**头节 625 的服务端位置**（节段距头节 ≤ 72px）。
   · 接管判据从「类型白名单数组」改为 **`SummonEntityTable.Of.ContainsKey`**（本体表为准），
     派生弹幕不在其列 → `ServerPosition` 保持 null → 命中几何不判（失败放行）。
   · 测试：`ServerAi_Covers_All_Body_Types`（**覆盖完整性不变量**：62 个本体必须全部拿到 ServerPosition、
     派生弹幕必须为 null —— 以后新增本体类型忘接管会被这条抓住）；并把 AI_062 那条用例里的 387 期望
     从「不受几何判定影响」更新为「同样被拒绝」，另补 374 派生弹幕仍放行的对照。
   **缺口现状**：**本体命中的归因原本完全没有几何要求** ——
   `FindOwnedSummonProjectile` 只按「归属 + 存活 + 召唤体系类型」挑一枚（原注释：原版 minion 的包 27
   上报频率不足，故不要求服务端缓存坐标与 NPC 重叠）。后果：只要玩家有**任意一枚**存活本体，
   就能对**全图任意** NPC 上报包 28（伤害受 `ceil(本体伤害×1.15)`、命中冷却与上游 `MaxDps` 约束，但**范围不受限**）。
   **本体侧已全部收口**（62 / 62 都有 `ServerPosition` + 1200px 可达圈）。
   **仍未收口**：**全部派生弹幕**（25 个类型）—— 它们由客户端模拟、服务端不持有位置，仍可对全图任意 NPC 报命中。
   要收口只能在 `ServerShots` 档由服务端生成它们（那时位置天然权威），或退一步给它们也用「父本体可达圈」判定。
...
- 配方表此前只收录 `SetupRecipes` 里**显式**写出的配方，缺少原版在末尾派生的两类配方：
  `CreateReverseWallRecipes`（墙 → 块）与 `CreateReversePlatformRecipes`（平台 → 材料）。
  缺这两类 → 玩家把墙 / 平台拆回材料（原版工作台旁的正常操作）会被判「配方不存在」而整窗回滚。

**二、生成器补齐（`gen-recipes.ps1`）**

- 结构化改造：配方先以结构体收集（产物 / 堆叠 / 站位 / 环境 / 材料），最后统一格式化 —— 派生规则需要读结构化字段。
- 新增两张抽取表：**物品 → `createTile` / `createWall`**（`Item.SetDefaults1..5` 的 switch-case，逐 case 取
  `createTile = N` / `createWall = N`，默认 -1）与 **`TileID.Sets.Platforms`**（7 个平台图格）。
- 按原版规则派生，**逐条镜像**：
  - 反向墙：产物 `createWall > 0` + 单材料（`requiredItem[1]` 为空）且材料 `createWall == -1` → 生成「材料 ← 墙」；
  - 反向平台：产物 `createTile` 属于平台图格 + 单材料 → 生成「材料 ← 平台」；
  - 两条规则**只复制 `requiredTile`，不复制液体环境**（与原版一致）。
- 产出：**3301 条**（基础 3193 + 派生 108）/ 34 个配方组。

...
> 最后更新：2026-09-19（W-2 第一步数据表收口；第二步 4 项全部落地；第三步 ServerAi：移动表 + `Static` + `PositionBound` + **位置权威与命中几何已覆盖 28 个本体**（AI_062 / AI_026 / AI_067）；834/834 通过）
...
2. **`ServerAi`**：本体跟随 / 驻守 / 索敌 / 节奏 / 暴击在服务端，包 27 只用于创建登记；
   ...
   **下一步**：把剩余 Body 族纳入服务端自算位置（aiStyle 54 的 317、aiStyle 66 的 387/388/533、120 的 623、
   121 的 625-628、156 的 755/946、158 的 759、164 的 831/970、169 的 864、206 的 1119、哨兵），并按族收紧几何；
   反向广播 + owner 特权回退按混合模型为「不做」（表现始终用客户端坐标）。
   **缺口现状（已部分收口）**：**本体命中的归因原本完全没有几何要求** ——
   ...
   **已对 28 个本体收口**（AI_062 六个 + AI_026 十个 + AI_067 十二个 = 服务端自算位置 + 1200px 可达圈）。
   **仍未收口**：其余 Body 族（317 Raven、387/388/533、623、625-628、755/946、759、831/970、864、1119、哨兵）
   与**全部派生弹幕** —— 它们仍可对全图任意 NPC 报命中。做法沿用同一模式：先把该族纳入服务端自算位置，
   再对其收紧几何。

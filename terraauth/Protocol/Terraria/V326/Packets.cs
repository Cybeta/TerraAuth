...
/// <summary>
/// NPC 命中确认（包 162）：无 payload。服务端每收到一次 NPC 命中（包 28）都必须回一个，
/// 客户端收到后调用 <c>NPC.AckDamage()</c> 把**最早入队**的那条待确认伤害出队。
/// <para>若不下发：客户端的待确认队列只增不减，其在收到包 23 时会以
/// <c>npc.life = 服务端血量 - 待确认伤害总和</c> 反复扣减，血量很快算成负数 ——
/// 表现为「客户端贴图消失（本地判死）、服务端该怪仍存活并继续造成接触伤害」的幽灵碰撞。</para>
/// </summary>
public sealed record NpcDamageAckPacket : INetworkPacket
{
    public PacketId Type => PacketId.NpcDamageAck;
}

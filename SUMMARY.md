...
>
> 生成日期：2026-09-18 ｜ 当前测试：以本次最终构建与测试结果为准；发布流水线同时验证 SQLite 和无 SQLite 后端。
...
7. **玩家弹幕命中必须关联服务端实体**：原版客户端可先在本地预测发射；服务端拒绝包 27 后，不会登记或广播该弹幕，但当前包 28 在找不到已登记弹幕时仍可能回退到手持远程武器的伤害上界校验。已识别的远程、魔法投射物命中必须验证同一玩家的服务端存活弹幕、合理空间碰撞与生命周期；近战和明确的兼容路径另行处理。
8. **提交后广播一致性**：客户端原包仍可能在仿真提交前广播，需让广播绑定已提交状态。
9. **容量与架构边界**：慢客户端策略、`WorldState` 并发契约、WorkerPool 收敛、协议元数据集中及程序集拆分。
...
- **导出的 `.wld` 验证程度**：已通过**逐格 round-trip** + **严格分段走查** + **双向原版互操作实测**
  （第二十轮：修正元数据文件类型字节后，服务端兼容性测试成功加载本服务端导出世界；
  反向本服务端加载兼容世界成功，逐段指针断言均通过，含 RLE 压缩图格段）。
